 ...
 *                                  невозможно подобрать набор бусин или
 *                                  после всех корректировок не удаётся
 *                                  достичь нужной длины.
*/
function braceletText(
    ...
    // После завершения цикла оцениваем итоговое отклонение.
    // Если оно выходит за пределы допустимых 2 мм, значит расчёт
    // не достиг требуемой длины и дальнейшее описание браслета
    // было бы некорректным — выбрасываем исключение.
    if (abs($delta) > 2) {
        throw new InvalidArgumentException(
            'Нужная длина браслета не достигнута'
        );
    }
