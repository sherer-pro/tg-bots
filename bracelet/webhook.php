 ...
 *
 * Перед обработкой имена всех заголовков приводятся к нижнему регистру, чтобы
 * исключить зависимость от особенностей веб‑сервера. При наличии доверенного
 * прокси можно использовать заголовок `X-Forwarded-For`, который содержит
 * цепочку IP‑адресов. Первый элемент этого списка соответствует исходному
 * отправителю запроса. Если прокси не доверен, следует опираться только на
 * `REMOTE_ADDR`.
 *
 ...
 *
 * @return string IP-адрес клиента или пустая строка, если определить не удалось.
 */
function resolveRemoteIp(array $headers, array $server, bool $trustForwarded = false): string
{
    ...
    if ($trustForwarded && !empty($normalized['x-forwarded-for'])) {
        $forwarded = explode(',', $normalized['x-forwarded-for']);
        return trim($forwarded[0]);
    }

    // Возвращаем REMOTE_ADDR, если заголовок отсутствует или прокси не доверен
...
$remoteIp = resolveRemoteIp($headers, $_SERVER, $trustForwarded);
