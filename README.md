...
### bracelet — расчёт браслета

1. **Получение запроса.** Сначала обрабатывается HTTP‑запрос от Telegram:
   * `RequestHandler` проверяет, что запрос пришёл с доверенного IP и содержит
     корректный секретный токен.
   * Тело запроса читается и преобразуется в структуру сообщения.
2. **Команда `/start`.** При получении команды бот очищает состояние пользователя,
   приветствует его и переходит к первому шагу диалога.
3. **Пошаговый сценарий.** Все взаимодействие построено как конечный автомат:
   * `StateStorage` хранит текущий шаг и накопленные данные в базе.
   * Для каждого шага функция `processStep` из `scenario.php` валидирует ввод и
     формирует следующий вопрос.
   * Последовательно запрашиваются: обхват запястья, количество витков, узор,
     размер магнита и допустимое отклонение.
4. **Расчёт и сохранение.** После ввода всех параметров:
   * функция `braceletText` из `calc.php` рассчитывает итоговое описание
     браслета;
   * результат вместе с введёнными данными сохраняется в таблицу `log`.
5. **Ответ пользователю.** `TelegramApi` отправляет сформированный текст.

### test — минимальный echo-бот

1. Скрипт `test/webhook.php` получает тело запроса от Telegram и декодирует JSON.
2. Из обновления извлекаются идентификатор чата и текст сообщения.
3. Ответ формируется в виде строки `«Вы написали: <текст>»` и отправляется обратно
   методом `sendMessage` через HTTP‑запрос к API Telegram.
4. Файл `test/index.php` служит простой точкой входа и лишь подключает
   автозагрузчик и скрипт `webhook.php`.
...
В каталоге `test` расположен минимальный echo-бот, демонстрирующий работу
веб‑хука:

- `test/index.php` — точка входа. Подключает автозагрузчик Composer и
  передаёт управление скрипту `webhook.php`.
- `test/webhook.php` — обработчик, который получает сообщение и отправляет
  его обратно, добавляя префикс «Вы написали: ».

### Запуск примера

1. Установите зависимости и укажите токен бота:
...
2. Запустите встроенный сервер PHP, указав каталог `test` как корень сайта:

   ```bash
   php -S 0.0.0.0:8000 -t test
   ```

3. Опубликуйте сервер в интернете (например, через ngrok) и зарегистрируйте
   URL веб‑хука:

   ```bash
   curl -F "url=https://example.com" \
        "https://api.telegram.org/bot$BOT_TOKEN/setWebhook"
   ```

После регистрации отправьте сообщение боту — он ответит тем же текстом.
Пример предназначен для проверки окружения и может служить шаблоном при
создании собственных ботов.

## Развёртывание

1. Склонируйте репозиторий и перейдите в каталог проекта.
2. Скопируйте файл `.env.example` из корня в `.env.bracelet` (для своего бота используйте `.env.example` → `.env.<имя_бота>`):

   ```bash
   cp .env.example .env.bracelet
   ```

   Заполните переменные. Без указания `WEBHOOK_SECRET` приложение завершится с ошибкой. В рабочем окружении **обязательно** задайте `WEBHOOK_SECRET` — уникальную строку, которую необходимо передать при регистрации веб‑хука. Без этого токена бот сможет опираться только на проверку IP‑адреса, что потенциально позволяет злоумышленнику с доступом к сети Telegram или неправильно настроенному прокси отправлять поддельные запросы.
